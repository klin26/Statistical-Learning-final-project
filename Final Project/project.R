...
summary(dat)
dat222 <- dat
dat222$Outcome <- as.factor(ifelse(dat222$Outcome=='0',3,4)) 
ggpairs(dat, aes(color = Outcome, alpha=0.5)) + scale_fill_manual(values = c("0" = "blue", "1" = "red"))
...
confusionMatrix(logi.class.pred,factor(dat.test$Outcome),positive = "1")
...
names(lda.pred)
confusionMatrix(lda.pred$class,factor(dat.test$Outcome),positive = "1")
...
confusionMatrix(qda.pred$class,factor(dat.test$Outcome),positive = "1")
...
set.seed(628)
rf.dia <- randomForest(Outcome ~ ., data = dat , subset = train , mtry = which(bestrf.para==max(bestrf.para),arr.ind=T)[1], importance = TRUE, ntree = 500 + 500 * which(bestrf.para==max(bestrf.para),arr.ind=T)[2])
...
rf.dia$importance %>%
  ...
  top_n(23,wt = MeanDecreaseAccuracy) %>%
...
set.seed (628)
boo.dia = gbm(I(Outcome == "1") ~ ., data = dat.train[folds != j,], distribution = "bernoulli", interaction.depth = tune$Var1[which(bestboo.para==max(bestboo.para),arr.ind=T)[1]], shrinkage = tune$Var2[which(bestboo.para==max(bestboo.para),arr.ind=T)[1]], n.tree = 500 + 500 * which(bestboo.para==max(bestboo.para),arr.ind=T)[2])
...
#cut
boo.prob <- predict(boo.dia,dat.test,type = "response")
boo.class.pred <- ifelse(boo.prob >= 0.3609467, '1', '0')
boo.class.pred <- factor(boo.class.pred)
perfor_dummy(boo.prob,boo.class.pred,factor(dat.test$Outcome),"Boosting")
...
#cut
svm.prob <- predict(svm.dia,dat.test, probability = TRUE)
svm.class.pred <- ifelse(attr(svm.pred, "probabilities")[, 1] >= 0.3609467, '1', '0')
svm.class.pred <- factor(svm.class.pred)
perfor_dummy(attr(svm.pred, "probabilities")[, 1],svm.class.pred,factor(dat.test$Outcome),"SVM ( radial )")
...
perfor_dummy(attr(svm.pol.pred, "probabilities")[, 2], factor(svm.pol.pred), factor(dat.test$Outcome), "SVM ( polynomial )", plot=T)

#cut
svm.pol.prob <- predict(svm.pol.dia,dat.test, probability = TRUE)
svm.pol.class.pred <- ifelse(attr(svm.pol.pred, "probabilities")[, 1] >= 0.3609467, '1', '0')
svm.pol.class.pred <- factor(svm.pol.class.pred)
perfor_dummy(attr(svm.pol.pred, "probabilities")[, 1],svm.pol.class.pred,factor(dat.test$Outcome),"SVM ( polynomial )")
...
as.data.frame(cbind(cutpoints=0.5,rbind('Logistic Regression','LDA','QDA','naiveBayes','tree','Random Forest','Boosting','SVM ( radial )','SVM ( poly )'),
...
as.data.frame(cbind(cutpoints=0.3609467,rbind('Logistic Regression','LDA','QDA','naiveBayes','tree','Random Forest','Boosting','SVM ( radial )','SVM ( poly )'),
                    rbind(perfor_dummy(logi.prob,logi.class.pred,factor(dat.test$Outcome),"logistic",plot=F),
                          perfor_dummy(lda.prob$posterior[,2],lda.class.pred,factor(dat.test$Outcome),"LDA",plot=F),
                          perfor_dummy(qda.prob$posterior[,2],qda.class.pred,factor(dat.test$Outcome),"QDA",plot=F),
                          perfor_dummy(nb.prob[,2],nb.class.pred,factor(dat.test$Outcome),"naiveBayes",plot=F),
                          perfor_dummy(tree.prob[,2],tree.class.pred,factor(dat.test$Outcome),"Tree",plot=F),
                          perfor_dummy(rf.prob[,2],rf.class.pred,factor(dat.test$Outcome),"Random Forest",plot=F),
                          perfor_dummy(boo.prob,boo.class.pred,factor(dat.test$Outcome),"Boosting",plot=F),
                          perfor_dummy(attr(svm.pred, "probabilities")[, 1],svm.class.pred,factor(dat.test$Outcome),"SVM ( radial )",plot=F),
                          perfor_dummy(attr(svm.pol.pred, "probabilities")[, 1],svm.pol.class.pred,factor(dat.test$Outcome),"SVM ( polynomial )",plot=F))))
